...
def parse_csdb_group_detail(html: str) -> Dict[str, Any]:
    ...
    if heading_font:
        ...
        abbr_tag = heading_font.find_next(string=lambda t: t and "(" in t and ")" in t)
        if abbr_tag:
    ...
    if creator_font:
        creator_link = creator_font.find("a", href=lambda h: h and "/scener/?id=" in h)
        if creator_link:
    ...
    for b in main.find_all("b"):
        if "User rating" in b.get_text():
            ...
            if td:
                ...
                stat_a = td.find("a", href=lambda h: h and "votestatistics.php" in h)
                if stat_a:
            ...
            if table:
                ...
                for tr in member_rows:
                    ...
                    if len(tds) >= 1:
                        ...
                        status_str = (
                            status.get_text(strip=True).strip("()") if status else None
                        )
        ...
        if "Releases" in b.get_text():
            ...
            if release_table:
                for row in release_table.find_all("tr"):
                    ...
                    if len(cols) >= 4:
                        release_link = cols[0].find(
                            "a", href=lambda href: href and "/release/?id=" in href
                        )
                        if release_link:
