...
class CSDBHandler(BaseHandler):
    ...
    def _download_file_by_id(
        self, file_id: int, target_path: Path, referer: Optional[str] = None
    ) -> Optional[str]:
        ...
        if "text/html" in content_type.lower():
            return (
                f"Failed to download {target_path.name}: received HTML instead of file"
            )
    ...
    def _format_find_result(self, result: dict, custom_section=None) -> str:
        ...
        if custom_section:
            ...
            output += fmt_items(
                result.get(key, []), result.get(count_key, 0), section, show_all_label
            )
        else:
            output += fmt_items(
                result.get("releases", []),
                result.get("release_count", 0),
                "release matches",
                "releases",
            )
            output += fmt_items(
        ...
        if state.get("zip_id") and state.get("zip_files"):
            ...
            for f in release_info["files"]:
                if fnmatch.fnmatch(f["name"], file_pattern):
                    ...
                    error = self._download_file_by_id(
                        int(f["id"]), file_path, referer=referer
                    )
                    if error:
                        output.append(error)
                    else:
    ...
    def _cd_into_zip(self, file_id: int, session_id: int) -> str:
        ...
        try:
            error = self._download_file_by_id(
                file_id,
                zip_path,
                referer=f"https://csdb.dk/release/?id={state.get('active_id')}",
            )
            if error:
                return error
    ...
    def _get_entry_info(
        self, entry_type: str, entry_id: int, session_id: int, depth: int = 2
    ) -> str:
        ...
        def format_group_output(group_data: dict, entry_id: int) -> str:
            ...
            if group_type and user_rating:
                type_rating_line = (
                    f"\x9fType\x9a: {group_type} | \x9fRating\x9a: {user_rating}"
                )
            elif group_type:
        ...
        try:
            params = {"type": entry_type, "id": entry_id, "depth": min(depth, 4)}
            ...
            if entry_type == "release":
                ...
                model = CSDBEvent(name=name, start_date=start_date, end_date=end_date)
    ...
    def _parse_and_execute(self, command: str, session_id: int) -> str:
        ...
        if cmd in ["ll", "dir"]:
            if state.get("active_dir") and state.get("active_id"):
                ...
                folders = ["group", "release", "scener", "event", "bbs", "sid"]
                return (
                    "\x9fCSDB root\x9a:\n"
        ...
        if cmd == "cd":
            ...
            # Aliases for cd <type>
            aliases = {"rel": "release", "grp": "group", "scn": "scener"}
            arg_lower = arg.lower()
            ...
            if (
                ...
                and state.get("active_dir") == "release"
                and state.get("active_id")
            ):
                release_info = self._get_parsed_release_info(state["active_id"])
                file_id = None
        ...
        if cmd in ["find", "ls"]:
            search_text = (
                arg if cmd == "find" else (state.get("last_find", "") if "ls" else "")
            )
            ...
            if state.get("active_dir"):
                ...
                return self._format_find_result(
                    result,
                    custom_section=(
                        state["active_dir"],
                        f"{state['active_dir']}s",
                        state["active_dir"] + "s",
                        state["active_dir"] + "_count",
                    ),
                )
            else:
                # Global search
