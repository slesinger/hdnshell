...
class TestC64TestClient(unittest.TestCase):
    ...
    def test_send_keypress(self):
        ...
        self.client.socket.recv.return_value = (
            MAGIC_BYTES + bytes([ResponseType.PETSCII_NULL_TERMINATED]) + b"abc\x00"
        )
        with patch("cloud_test_client.Petscii.ascii2petscii", return_value=65):
            with patch(
                "cloud_test_client.Petscii.petscii2ascii", side_effect=lambda b: b
            ):
    ...
    def test_send_text(self):
        self.client.socket.recv.return_value = (
            MAGIC_BYTES + bytes([ResponseType.PETSCII_NULL_TERMINATED]) + b"hello\x00"
        )
    ...
    def test_scenario_csdb_find_error(self):
        ...
        responses = [
            MAGIC_BYTES
            + bytes([ResponseType.PETSCII_NULL_TERMINATED])
            + b"csdb mode\x00",
            MAGIC_BYTES
            + bytes([ResponseType.PETSCII_NULL_TERMINATED])
            + b"error: 'name'\x00",
    ...
    def test_cd_to_meetro(self):
        ...
        responses = [
            ...
            MAGIC_BYTES
            + bytes([ResponseType.PETSCII_NULL_TERMINATED])
            + b"Switched to group directory.\x00",
            # 'pwd'
            MAGIC_BYTES
            + bytes([ResponseType.PETSCII_NULL_TERMINATED])
            + b"c:/group\x00",
