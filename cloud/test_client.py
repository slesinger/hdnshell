...
class C64TestClient:
    ...
    def send_text(self, text: str):
        ...
        packet = MAGIC_BYTES + bytes([CommandID.TEXT_INPUT]) + petscii_bytes + bytes([0x00])
    ...
    def print_response(self, response: bytes):
        ...
        if resp_type == ResponseType.PETSCII_NULL_TERMINATED:
            ...
            if len(petscii_data) > 0:
                ...
                try:
                    ascii_bytes = bytes([Petscii.petscii2ascii(b) for b in petscii_data])
...
def main():
    ...
    parser.add_argument('--port', type=int, default=6464, help='Server port (default: 6464)')
