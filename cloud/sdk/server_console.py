...
# _TOAST_* constants are re-exported here for backwards compatibility
# with anything importing them from this module.
from .toaster import (
    paint_toaster,
    _toast_word_wrap,
    _TOAST_MAX_CONTENT_WIDTH,
    _TOAST_PADDING,
    _TOAST_SC_TL,
    _TOAST_SC_TR,
    _TOAST_SC_BL,
...
#
# The actual implementation now lives in sdk.toaster (shared with the
# tutorials coach overlay -- see TUTORIALS_PLAN.md §5.3/§9 Phase 2). These
# names stay here as thin aliases so existing call sites/tests importing
# `_render_toast_box`/`_toast_word_wrap` from sdk.server_console keep
# working, byte-for-byte identical to the pre-factor behavior.
# ======================================================================
...
def _render_toast_box(scr: bytearray, col: bytearray, text: str) -> None:
    ...
    Thin alias for `sdk.toaster.paint_toaster` -- kept so existing call
    sites/tests referencing `sdk.server_console._render_toast_box` keep
    working. See `sdk.toaster.paint_toaster` for the implementation.
    """
