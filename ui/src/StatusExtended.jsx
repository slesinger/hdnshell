      ...
      try {
        ...
        if (!cancelled) {
          setStatus({
            ...
            "hdnsh.conf_present": Boolean(payload?.["hdnsh.conf_present"])
          });
        }
      } catch (err) {
        if (!cancelled) {
    ...
    try {
      ...
      if (statusResponse.ok) {
        ...
        setStatus({
          ...
          "hdnsh.conf_present": Boolean(statusPayload?.["hdnsh.conf_present"])
        });
      }
    } catch (err) {
  ...
  return (
    <section className="status-extended">
      <div className="d-flex flex-column flex-md-row align-items-start justify-content-between gap-3 mb-3">
        <div>
          <h2 className="h5 mb-1">C64 Ultimate status</h2>
          <p className="text-muted mb-0">
            Latest status checks for your C64 Ultimate services and ROM setup.
          </p>
        </div>
        <div className="text-md-end">
          <div className="text-muted small">Last known IP</div>
          <div className="fw-semibold">
            {lastC64Ip?.trim() ? lastC64Ip : "Not detected yet"}
          </div>
        </div>
      </div>
        ...
        <div className="row g-3">
          <div className="col-12 col-lg-6">
            <div className="border rounded-4 p-3 h-100">
              <h3 className="h6">Network services</h3>
            ...
            <div className="border rounded-4 p-3 h-100">
              ...
              <ul className="list-unstyled mb-0">
                ...
                <li className="d-flex align-items-center gap-2">
                  <span
                    className={`status-dot ${
                      status["hdnsh.conf_present"] ? "ok" : "danger"
                    }`}
                  />
                  <span>hdnsh.conf</span>
                </li>
              </ul>
            </div>
          </div>
