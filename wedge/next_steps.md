...
| Step ID | Change | HW Test Expectation | Status |
|---|---|---|---|
| BB-pre | Prove a bank5 reserve pocket is reachable from `api_21` **at cold boot** (border-flash stub via `call_bank5` trampoline) + prove the bank5 `$8023-$80FF` dead pocket is TASS-safe + prove CHROUT from a mapped pocket | Cold boot → **border colour shifted** AND the `CYBERPUNX RETRO REPLAY 64KB - 3.8P` line still prints (now from bank5); READY. normal; **stock sweep + TASS/TMP launch/exit intact** (probe must not corrupt the REU image); watch for extra border flashes on RUN/STOP-RESTORE (api_21 is the shared version-line printer — cosmetic, expected) | 🔨 **built+byte-verified 2026-07-14** — diff vs pre-BB baseline = **bank3 `$9FC8-$9FF3` (43 B) + bank5 `$8023-$805A` (55 B) ONLY**; banks 0/1/2/4/6/7 byte-identical; TMP payload `$8100` (`jmp $80B7`) intact. `build/archive/BB-pre.bin`. See conversion_log3.md §31. ✅ **HW tested 2026-07-15 — all good** (border shifted to grey, RETRO REPLAY line prints from bank5, TASS/stock sweep intact: cold-boot trampoline + bank5 `$8023-$80FF` pocket + CHROUT-from-pocket all proven) |
| BB1 | NON-DESTRUCTIVE `reu_detect` (read-save-restore ladder) + decimal-MB print in the bank5 pocket; boot line 2 → `<n>M REU  RR 3.8P` | Boot line 2 shows correct REU size (e.g. `16M REU  RR 3.8P`) in place of the RETRO REPLAY line; **TASS/TMP launch & exit still work** (the probe must leave the REU image intact); stock sweep | 🔨 **built+byte-verified 2026-07-15** — bank5 `$8023-$80FD` (207 B; probe+print+table fill the dead pocket); **bank3 identical to BB-pre** (trampoline unchanged); banks 0/1/2/4/6/7 identical; TMP payload `$8100` intact; fits with 2 B margin. Assumes REU present (no-REU graceful → BB2). `build/archive/BB1.bin`. See conversion_log3.md §31. ✅ **HW tested 2026-07-15 — all good** (boot line 2 = `16M REU  RR 3.8P`; REU correctly probed as 16 MB; TASS/TMP launch+exit + stock sweep intact → non-destructive probe leaves the REU image untouched) |
| BB2-pre | **Open bank7** (fresh bank, never mapped) — BB1's boot-line relocated there; api_21 trampoline repointed bank5→bank7 (`$88`→`$98`); **bank5 reverted to stock**. Needed because bank5/6 pockets are full and BB2's UCI code needs bank7's 546 B of dead pockets | Boot line 2 = `16M REU  RR 3.8P` (now printed from **bank7**); READY. normal; **stock sweep + TASS/TMP launch/exit intact** — the line appearing correctly + TASS launching IS the proof bank7 maps safely at cold boot + its `$8023-$80FF` pocket is TASS-safe | 🔨 **built+byte-verified 2026-07-15** — diff vs pre-BB baseline = **bank3 `$9FC8-$9FF3` (43 B, tramp now maps `$98`) + bank7 `$8023-$80FD` (207 B) ONLY**; **bank5 reverted (identical to baseline)**; banks 0/1/2/4/6 identical; bank7 TMP payload `$8100+` intact. `build/archive/BB2-pre.bin`. See conversion_log3.md §31. ✅ **HW tested 2026-07-15 — all good** (boot line 2 = `16M REU  RR 3.8P` from bank7; TASS/stock intact → bank7 maps safely at cold boot + its pocket is TASS-safe) |
| BB2 | UCI-version block `uci_ver` in bank7 `$9F58` pocket (+ `reu_xfer`/"UCI " text relocated to `$9E00`; all same-bank jsr): self-contained UCI DOS1 IDENTIFY (bank-independent `$df1c-$df1f`, bounded loops) + scan reply for first `V` → boot line `<n>M REU UCI Vx.y RR 3.8P`; prints nothing if no Ultimate replies (line stays `<n>M REU RR 3.8P`) | Boot line 2 shows `16M REU UCI V1.2 RR 3.8P` (version = your Ultimate DOS); Command Interface off → `UCI` field omitted, no hang; `status` still works after boot (UCI left idle); stock sweep + TASS/TMP | 🔨 **built+byte-verified 2026-07-15** — diff vs pre-BB baseline = **bank3 `$9FC8-$9FF3` (43 B, unchanged from BB2-pre) + bank7 `$8023-$9FD9` (350 B across 3 pockets)**; banks 0/1/2/4/5/6 identical; bank7 TMP payloads `$8100-$9DFF` + `$9E9D+` byte-identical; `uci_ver` fully disassembly-verified. `build/archive/BB2.bin`. See conversion_log3.md §31. ⚠️ **HW tested 2026-07-15: REU+RR OK but UCI field MISSING** (trimmed `uci_ver` pushed IDENTIFY without waiting for UCI idle — works at prompt via `status`' idle_kick, fails at boot). **→ BB2b:** restored `uv_widl` wait-idle + busy-wait (now matches `status` path; `uci_ver` 160 B) + `RR 3.8P`→`CPX RR 3.8P` (Honza). Line → `16M REU UCI Vx.y CPX RR 3.8P`. bank7 401 B, disassembly-verified; `build/archive/BB2b.bin`. ✅ **HW tested 2026-07-15 — all good** (Honza confirmed) |
| BB3 | Docs: reconcile `installation_alternative.md` "Verify it works" to actual RR-wedge screen (line 1 = `BASIC V2`, line 2 = `16M REU UCI V1.2 RR 3.8P`) | Manual matches real boot screen | ⬜ (after BB-L1) |
| BB-L1 | **Line 1 banner text** — `**** COMMODORE 64 BASIC V2 ****` → `**** COMMODORE 64 SHELL V1 ****`. Reopens the 2026-07-14 "NOT DOING" decision, but via a NEW zero-bank1-risk mechanism (found 2026-07-15): `$e422` (KERNAL) still runs 100% unmodified; `bank03_api_21`'s existing cold-boot trampoline (already proven by BB-pre/BB1/BB2/BB2b) does a SECOND excursion — into bank5's now-empty `$8023` pocket (reverted to stock since BB2-pre, unused since) — that overwrites the 31 already-printed screen bytes at their fixed boot position (row 1, col 4 of the default `$0400` screen) with our own text, before the bank7 REU/UCI/RR line prints. Bank1/KERNAL byte-identical; no new RAM trampoline home (reuses `$0378`, live-patching 1 byte between the two sequential calls). Full design + exact screen-code table: conversion_log3.md §32. | Cold boot line 1 reads `**** COMMODORE 64 SHELL V1 ****`; line 2 (`16M REU UCI Vx.y CPX RR 3.8P`) + `NNNNN BASIC BYTES FREE` unchanged; stock sweep + TASS/TMP intact (bank5 `$8023` pocket already proven TASS-safe by BB-pre/BB1) | 🔨 **built+byte-verified 2026-07-15** — diff vs BB2b baseline = **bank3 `$9FCB-$9FF1` (30 B, the retarget-and-second-jsr insert + bb_tramp table shifting 13 B down, still `<= $9FF5`) + bank5 `$8023-$804D` (43 B: `l1_poke`+`l1_text`) ONLY**; **banks 0/1/2/4/6/7 byte-identical** (bank1/KERNAL untouched — confirmed byte-for-byte); both changed regions manually disassembled from the built `.bin` and confirmed to match the intended bytes exactly (operands, branch targets, `l1_text` = the correct 31 screen-code bytes). `build/archive/BB-L1.bin`. — **awaiting HW test** |
...
| 22 | Verify pass-throughs + final sweep | `i:`, `m:`, `:` still work; all new commands once more; manual wording vs actual; update docs | 🔨 **docs reconciled 2026-07-13** — pass-throughs unaffected (only `#`-lines changed; cloud suite 161 passed); manual fixed (dos.md #u/#v + mount roots + auto-cd + `ll` prefix rule; file-operations.md + memory-operations.md device sets; using-the-shell.md); search-index regenerated; analysis-doc overlay note. **Final HW sweep owed (Honza)** |
