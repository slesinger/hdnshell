...
- **2026-07-12:** created this document. Improvement pass (wedge untouched):
  - **UI:** rebranded HDN Cloud → HDN Server; removed leftover scaffolding
    copy on the home page; hash routing (deep-linkable pages incl.
    `#/docs/<slug>`, working back/forward); Menu/Reset/Reboot/Power off moved
    to a "Machine" dropdown with confirmations; machine actions now show
    success/failure banners; C64 palette + C64 Pro Mono wordmark.
  - **Server:** string rebrand across cloud/; every outbound HTTP call now has
    a timeout; fixed a socket leak in `sdk/network_helper.py`; deduped the
    GitHub latest-release fetch; `ensure_rom` accepts POST; print → logging.
    Tests: 149 passed / 1 skipped, 0 failures.
  - **Docs:** new HDN Server developer README (`cloud/README.md`); deprecation
    banner on `README-v2.md`; "What you need" hardware table in
    `installation.md`; terminology sweep (no "Cloud Server" left in the
    manual); web-UI machine controls + deep links documented in
    `cloud-integration.md`; clarified that `cloud/static/` is generated by
    `npm run build` in `ui/` (no manual mirror sync needed).
