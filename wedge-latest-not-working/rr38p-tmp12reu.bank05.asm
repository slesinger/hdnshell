...
.const UII_CTL_ABORT = $04    // abort the command in flight (reference
                              // uii_abort() in docs/inspiration/ultimate_lib.c)
...
// net_wait_idle -- ROUND 11: lives in the boot pocket now (it grew from
// "wait for idle" into "MAKE idle", see the full body + rationale next to
// boot_banner). Still the first call of every UCI transaction (dos_begin,
// nw_start, net_connect, net_close, net_read_response), still preserves X.
...
// ---------------------------------------------------------------------------
// net_wait_idle -- ROUND 11 REWRITE: from "wait for idle" to "MAKE idle".
//
// The UCI state machine lives on the Ultimate side and is NOT reset by a C64
// reset. If the previous session ended mid-transaction (reset/freeze while a
// command was in flight, a crash before an accept, the C64U's own boot
// activity), the interface still holds an unaccepted response block when the
// C64 comes back up. Every transaction after that runs OFF BY ONE: each
// command reads the *previous* command's response. For the short DOS commands
// the shell uses, that skew is nearly invisible (statuses are all "00", #h
// still prints a plausible path) until something desyncs harder and the
// firmware acts on garbage -- the reported deterministic "fresh start, #h,
// cd / -> font garbage + freeze menu" shape, which an `ll` in between
// "cured" ONLY because wd_uii_dir's read loop happens to drain + accept
// pending blocks in a loop. This is a known-enough condition that the
// reference sample (docs/inspiration/ultimatedos-samples/u-sample.c) opens
// with exactly this guard: if data is available before the first command,
// abort the previous one.
//
// So instead of passively waiting (round 10 even made the wait give up after
// 3s and PUSH INTO the dirty interface -- worse), this now actively cleans:
// wait out a busy command (bounded), then drain leftover response data +
// status and ACCEPT the stale block; if the interface stays busy past the
// budget, ABORT the stuck command (control bit 2, reference uii_abort()).
// Up to 6 rounds, so several queued stale blocks (or one large one, drained
// 256 bytes a round) all clear. The fast path -- interface idle and clean,
// the 99% case -- is a single status read and a branch.
//
// Every UCI transaction enters through here (dos_begin, nw_start,
// net_connect, net_close, net_read_response's poll loop), and so does the
// boot banner's IDENTIFY -- which means a cold start now begins by clearing
// whatever the pre-reset session left behind. Preserves X (dos_begin keeps
// its target register there). Uses A/Y + w_to.
// ---------------------------------------------------------------------------
net_wait_idle:
    lda #$06                   // cleanup-round budget
    sta w_to+1
nwi_round:
    lda #$40                   // per-round busy allowance: $40*256 polls (~250ms)
    sta w_to
    ldy #$00
nwi_busy:
    lda UII_STATUS
    and #UII_ST_STATE
    cmp #$10                   // command still executing?
    bne nwi_settled
    iny
    bne nwi_busy
    dec w_to
    bne nwi_busy
    lda UII_CONTROL            // stuck busy a whole round: abort it
    ora #UII_CTL_ABORT
    sta UII_CONTROL
    jmp nwi_next
nwi_settled:
    lda UII_STATUS
    and #UII_ST_STATE + UII_ST_STATAV + UII_ST_DATAAV
    beq nwi_done               // idle, nothing pending: clean -- the fast path
    ldy #$00                   // stale response block: drain its data...
nwi_dd:
    lda UII_STATUS
    and #UII_ST_DATAAV
    beq nwi_ds
    lda UII_RESP
    iny
    bne nwi_dd                 // (256 bytes/round; bigger blocks clear over rounds)
nwi_ds:
    ldy #$00                   // ...drain its status...
nwi_ds2:
    lda UII_STATUS
    and #UII_ST_STATAV
    beq nwi_acc
    ...
    bne nwi_ds2
nwi_acc:
    lda UII_CONTROL            // ...and accept it, unblocking the next one
    ora #UII_CTL_ACC
    sta UII_CONTROL
nwi_next:
    dec w_to+1
