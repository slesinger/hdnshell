...
.const reu_save        = $0201 // 9 bytes ($0201-$0209): original REU bytes saved across
                                 ...
                                 // Boot-time only; the same page doubles as wc_buf
...
//
// wc_buf history (carried fix from round 10, 2026-07-06): it lived at
// $0110-$014f in round 6 -- WRONG, and the cause of the "any fast load after
// an ll/dir/cp crashes into the freeze menu" hangs. The cart firmware keeps
// RESIDENT machinery in the low stack page: a 23-byte helper routine at
// $0120-$0136 (installed from bank01 ROM $8df2 by the directory-print path
// at $8db7, then *called on every fastload* -- $8926's loader does jsr $de9d,
// whose trampoline body is jsr $0120) plus variables at $0134/$0135,
// $0158/$0159, $015e (a SAVED STACK POINTER that gets TXS'd!) and $0168. Any
// ll/dir/cp that buffered an entry there destroyed that code, and the next
// F1/%/LOAD executed garbage at $0120 -> BRK -> CBINV ($decd) -> RR freeze menu.
...
.const w_to         = $03fc   // +$03fd: UCI wait-loop timeout countdown (carried
                              //   fix from round 10) -- $03fc-$03ff sits past the
                              //   datassette buffer ($033c-$03fb) and below the
                              //   screen, plain unused RAM on a stock C64. If a
...
wedge_words:
    ...
    .byte $43,$d0                            // CP
    .byte <cmd_cp, >cmd_cp
    // MEMCPY dispatch entry DISABLED (round-12 rebuild, 2026-07-08): the
    // command never worked -- KERNAL LOAD/SAVE dispatch through the cart's
    // ILOAD/ISAVE vectors ($0330/$0332 -> $de6d/$de6f), whose trampoline
    // bytes only exist in bank01's mirror; cmd_memcpy runs with bank05
    // paged, so the jsr executed $00 bytes -> BRK -> RR freeze menu. The
    // round-10 fix (a RAM bank-bracket stub) is deliberately not carried
    // into this rebuild; a typed `memcpy` now chat-forwards like any other
    // unrecognized line. Re-enable by restoring these two lines together
    // with round 10's mcio_stub/mcio_prep machinery.
...
wd_chat:
    ...
    jsr net_close
    rts
wdc_fail_close:                // carried fix (round 10): a failed write/read used to
    jmp net_close              // leave the socket open -- close it so retries start clean
net_fail:
    rts
...
send_cli:
    ...
    jsr nw_send_shadow
    jsr nw_finish
    bcs wdc_fail_close         // carried fix (round 10): close the socket on a failed write
    lda #$00
    sta w_quiet
    jsr net_read_response
    jsr net_close
...
nrr_loop:
    ...
    bcs nrr_fail               // carried fix (round 10): bounded -- a wedged read gives up
...
// CARRIED FIX (round 10): every UCI wait is BOUNDED. These used to be
// unconditional busy-loops -- if the command interface was ever left busy /
// mid-handshake (firmware hiccup, a command aborted halfway, the Ultimate
// menu freezing the machine mid-transaction), the next wedge command spun
// here forever with interrupts off: dead cursor, dead keyboard, only the
// freeze button left. That is exactly the reported "m:/cd started hanging
// with no crt or server change" failure shape. On timeout they simply
// return -- the pushes and status/accept handshakes that follow every wait
// already surface errors (?ERR / silent fail / NOT REACHABLE) and BASIC
// gets its READY back.
...
net_accept:
    ...
    ora #UII_CTL_ACC
    sta UII_CONTROL
    lda #$00                   // carried fix (round 10): the accept handshake is
    sta w_to                   // bounded too (~1s) -- same rationale as the waits above
    ldy #$00
na_wait:
    lda UII_STATUS
    and #UII_CTL_ACC
...
// ===========================================================================
// Cold-start boot banner (round 7) -- replaces the stock "**** COMMODORE 64
// BASIC V2 ****" / BYTES FREE print (bank01's cold-start hook calls here
// instead of the real $e422). Prints "**** COMMODORE 64 SHELL V2 ****", then
// a line with the detected REU size and the Ultimate DOS identify string,
// then returns -- BASIC's own cold-start tail continues into the stock
// READY. exactly as before (no round-8 IERROR install here: this rebuild
// stops at round 7, stock prompt behavior is the baseline).
// ===========================================================================
boot_banner:
...
bb_dos:
    ...
    jmp print_msg               // rts from there, back to bank01's boot_stub
...
//
// CARRIED FIX (round 10) -- the probe is NON-DESTRUCTIVE. The original
// round-7 version wrote a byte to offset 0 of EVERY 64KB bank on every cold
// boot, permanently corrupting whatever lives in the REU -- and on this
// build ("tmp12reu") the REU is not empty: Turbo Macro Pro's launch image
// lives there (the cart's `\` handler at $9d1d DMAs REU $0800-$ffff into
// C64 RAM and jumps into it, and the TASS install path manages that image).
// Poking the REU at boot is both a transparency violation (the shell must
// leave the machine as if it weren't there) and the prime suspect for "TASM
// after reset dies into the freeze menu". Now: every REU byte the probe
// touches is read out first (reu_save) and written back afterwards --
// including the aliased case, where a rung write actually lands on bank 0 --
// and the ladder is powers of two (1,2,4,...,128), so at most 9 bytes are
// touched at all instead of 256. Real REU sizes are powers of two, so the
// result is exact.
reu_detect:
