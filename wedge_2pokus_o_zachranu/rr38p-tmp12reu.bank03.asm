...
// text + CR) that shows up below the HDN boot banner on every cold boot.
// Suppressed (wedge round 7 hardware feedback, 2026-07-06) by swapping the
// 3-byte jsr for a 3-byte jmp straight to this routine's own rts, skipping
// print and string alike. The string bytes stay in place as data (never
// executed, never printed) so nothing after them shifts. The bytes after
// this jsr were previously misdecoded here as opcodes -- they are the
